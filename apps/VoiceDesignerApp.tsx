...
const SOUND_EFFECTS_OPTIONS = [
  ...
  { value: 'spacious_echo', label: 'Spacious Echo' },
  { value: 'auditorium_echo', label: 'Auditorium Echo' },
  { value: 'lofi_telephone', label: 'LoFi Telephone' },
];
...
const convertHexAudioToBlob = (hexAudio: string, mimeType = 'audio/mpeg'): Blob => {
  ...
  if (!cleanHex || cleanHex.length % 2 !== 0 || /[^\da-f]/i.test(cleanHex)) {
    throw new Error('MiniMax returned malformed HEX audio data');
  }
...
const fetchRemoteAudioBlob = async (sourceUrl: string): Promise<Blob> => {
  ...
  if (!blob.size) throw new Error('Audio downloaded as an empty file');
  return blob;
};
...
const VoiceDesignerApp: React.FC = () => {
  ...
  const handleLoadVoices = async () => {
    ...
    if (!apiKey) return addToast('Please configure the MiniMax API Key in Settings first', 'error');
    ...
    try {
      ...
      addToast(`Loaded ${allVoices.length} voices`, 'success');
    } catch (err: any) {
      addToast(err?.message || 'Failed to load voices', 'error');
    } finally {
  ...
  const buildPayload = (text: string, languageBoost?: string) => {
    ...
    if (validTimbers.length === 0) {
      addToast('Please add at least one voice', 'error');
      return null;
    }
  ...
  const handlePreview = async () => {
    ...
    if (!text) return addToast('Please enter preview text', 'error');
    ...
    try {
      ...
      if (cached) {
        ...
        if (!response.ok || (typeof statusCode === 'number' && statusCode !== 0)) {
          throw new Error(data?.base_resp?.status_msg || `Call failed (HTTP ${response.status})`);
        }

        const audioRaw = data?.data?.audio;
        if (!audioRaw || typeof audioRaw !== 'string') throw new Error('The API did not return audio data');
  ...
  const handleBakeVoice = async () => {
    ...
    try {
      ...
      addToast('Synthesizing a long audio clip and cloning the voice, please wait...', 'success');
      ...
      if (!res.ok || data?.error) {
        throw new Error(data?.error || `Failed to fix the voice (HTTP ${res.status})`);
      }

      // Replace timber_weights with the new fixed voice_id
      setTimberWeights([{ id: `baked-${Date.now()}`, voice_id: customVoiceId, voice_name: `Fixed Voice (${customVoiceId})`, weight: 1 }]);
      addToast(`Voice fixed! voice_id: ${customVoiceId}`, 'success');
      ...
      addToast(err?.message || 'Failed to fix the voice', 'error');
    } finally {
      setIsBaking(false);
    }
  };
  ...
  // ── Apply to character ──
  const handleApply = () => {
    if (!selectedChar) return addToast('No character selected', 'error');
    const validTimbers = timberWeights.filter(tw => tw.voice_id.trim());
    if (validTimbers.length === 0) return addToast('Please add at least one voice', 'error');
    ...
    const updatedProfile = {
      ...
      voiceProfile: {
        ...
        voiceId: validTimbers.length === 1 ? validTimbers[0].voice_id.trim() : '',
        minimaxParamVersion,
        voiceName: validTimbers.length === 1 ? (validTimbers[0].voice_name || validTimbers[0].voice_id) : `Mixed Voice (${validTimbers.length})`,
    ...
    trackEvent('Apply Crafted Voice to Character');
    addToast(`Applied the crafted voice to "${selectedChar.name}"`, 'success');
  };
  ...
  return (
    <div className="h-full w-full bg-gradient-to-b from-violet-50 to-white flex flex-col">
      ...
      <header className="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-white/80 backdrop-blur-sm shrink-0" style={{ paddingTop: 'max(0.75rem, var(--safe-top))' }}>
        <div>
          <h2 className="text-sm font-bold text-slate-800">Craft Voice</h2>
          <p className="text-[10px] text-slate-400">
            {selectedChar ? `Design a voice strand for "${selectedChar.name}"` : 'MiniMax Voice Designer'}
          </p>
        </div>
        <div className="flex gap-2">
          <button onClick={handleApply} className="text-[10px] px-3 py-1.5 rounded-full bg-violet-500 text-white font-bold flex items-center gap-1 active:scale-95 transition-transform">
            <FloppyDisk size={12} weight="bold" /> Apply
          </button>
          <button onClick={() => closeApp()} className="text-[10px] px-3 py-1.5 rounded-full bg-slate-100 text-slate-600 font-bold">Close</button>
        </div>
      </header>

      {/* Tabs */}
      <div className="flex border-b border-slate-100 bg-white/60 shrink-0">
        <button onClick={() => { setTab('mix'); trackEvent('Switch Voice Designer Tab', { tab: 'mix' }); }} className={`flex-1 py-2.5 text-xs font-semibold transition-colors relative ${tab === 'mix' ? 'text-violet-600' : 'text-slate-400'}`}>
          Mix Voices
          {tab === 'mix' && <div className="absolute bottom-0 left-1/4 w-1/2 h-0.5 bg-violet-500 rounded-full" />}
        </button>
        <button onClick={() => { setTab('modify'); trackEvent('Switch Voice Designer Tab', { tab: 'modify' }); }} className={`flex-1 py-2.5 text-xs font-semibold transition-colors relative ${tab === 'modify' ? 'text-violet-600' : 'text-slate-400'}`}>
          Fine-tune Voice
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="bg-white rounded-2xl p-4 border border-violet-100 shadow-sm space-y-2.5">
          <div className="flex items-center justify-between gap-2">
            <span className="text-xs font-bold text-slate-700">MiniMax Synthesis Parameters</span>
            <span className="text-[9px] text-slate-400">Saved per character</span>
          </div>
          ...
          <p className="text-[10px] leading-relaxed text-slate-400">
            {minimaxParamVersion === 'natural-v2'
              ? 'Uses the same parameters for preview, chat, dates, and calls; relies on the native punctuation-driven pacing of the model instead of forcing a pause at every punctuation mark.'
              : 'Fully preserves the original parameters, auto-pause, and limiting rules; older characters keep using this tier by default.'}
          </p>
        </div>
          ...
          <>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-700">Voice Recipe</span>
                <div className="flex gap-2">
                  {availableVoices.length === 0 && (
                    <button onClick={handleLoadVoices} disabled={isLoadingVoices}
                      ...
                      {isLoadingVoices ? 'Loading...' : 'Load Voice Library'}
                    </button>
                  )}
                  <button onClick={addTimberSlot} className="text-[10px] px-2 py-1 rounded bg-emerald-50 text-emerald-600 font-bold hover:bg-emerald-100 flex items-center gap-0.5">
                    <Plus size={10} weight="bold" /> Add Strand
                  </button>
                </div>
              </div>
              <p className="text-[10px] text-slate-400">Add multiple voices and adjust their weights to mix a unique voice strand. A single voice also works — fine-tune it later under "Fine-tune Voice".</p>
            </div>
            ...
            <div className="flex items-start gap-1.5 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
              ...
              <p className="text-[10px] text-amber-700 leading-relaxed">
                It is recommended to <b>only mix voices from the same language</b>. For example, for a Korean-speaking character, only pick Korean voices to mix — mixing in voices from other languages can easily give the character an <b>accent</b>. Once mixed, use the multi-language samples above to preview and confirm.
              </p>
            </div>
            ...
            {timberWeights.map((tw, index) => (
              <div key={tw.id} className="bg-white rounded-2xl p-3 border border-slate-100 shadow-sm space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-slate-500">Voice Strand #{index + 1}</span>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="Enter a voice_id or pick from the library"
                  />
                  {availableVoices.length > 0 && (
                    <button onClick={() => { setPickingForIndex(index); setShowVoicePicker(true); trackEvent('Open Voice Picker'); }}
                ...
                <Slider label="Weight" value={tw.weight} min={1} max={100} step={1} onChange={v => updateTimberWeight(index, v)} />
              </div>
            ))}
            ...
            {timberWeights.length === 0 && (
              <div className="text-center py-8 text-slate-300">
                ...
                <p className="text-xs">No voice strands yet, tap "Add Strand" above to start</p>
              </div>
            )}
          </>
        )}
        ...
        {tab === 'modify' && (
          <>
            <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm space-y-3">
              <span className="text-xs font-bold text-slate-700">Basic Parameters</span>
              <Slider label="Speed" value={speed} min={0.5} max={2} step={0.1} onChange={setSpeed} unit="x" />
              <Slider label="Volume" value={volume} min={0} max={2} step={0.1} onChange={setVolume} />
              <Slider label="Base Pitch" value={pitch} min={-12} max={12} step={1} onChange={setPitch} />
            </div>

            <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm space-y-3">
              <span className="text-xs font-bold text-slate-700">Voice Modifiers</span>
              <Slider label="Pitch Offset" value={modifyPitch} min={-100} max={100} step={1} onChange={setModifyPitch} />
              <Slider label="Intensity" value={modifyIntensity} min={-100} max={100} step={1} onChange={setModifyIntensity} />
              <Slider label="Timbre" value={modifyTimbre} min={-100} max={100} step={1} onChange={setModifyTimbre} />
              <div className="space-y-1">
                <span className="text-[11px] text-slate-500">Sound Effect</span>
            ...
            <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm space-y-3">
              <span className="text-xs font-bold text-slate-700">Emotion</span>
              <div className="flex flex-wrap gap-1.5">
                {['', 'happy', 'sad', 'angry', 'fearful', 'disgusted', 'surprised', 'calm'].map(em => (
                  <button key={em} onClick={() => setEmotion(em)}
                    ...
                    {em === '' ? 'Auto' : em === 'happy' ? 'Happy' : em === 'sad' ? 'Sad' : em === 'angry' ? 'Angry' : em === 'fearful' ? 'Fearful' : em === 'disgusted' ? 'Disgusted' : em === 'surprised' ? 'Surprised' : 'Calm'}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm space-y-2">
              <span className="text-xs font-bold text-slate-700">TTS Model</span>
        ...
        <div className="bg-gradient-to-r from-violet-50 to-purple-50 rounded-2xl p-4 border border-violet-100 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-violet-700">Preview</span>
            <span className="text-[9px] text-violet-400">Tap a language to switch samples</span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {PREVIEW_SAMPLES.map(s => (
              <button key={s.code}
                onClick={() => { setPreviewLang(s.code); setPreviewText(s.text); trackEvent('Switch Preview Language Sample', { lang: s.code }); }}
          ...
          <div className="flex gap-2">
            <button onClick={handlePreview} disabled={isGenerating}
              ...
              <SpeakerHigh size={14} weight="bold" /> {isGenerating ? 'Synthesizing...' : 'Preview'}
            </button>
            {audioUrl && (
              <>
          ...
          {audioUrl && timberWeights.filter(tw => tw.voice_id.trim()).length > 1 && (
            <div className="space-y-1.5">
              <p className="text-[10px] text-emerald-600/80 text-center">
                <Check size={12} weight="bold" className="inline" /> The mixed voice strand is ready — tap "Apply" to use it directly. It mixes in real time during calls, matching this preview.
              </p>
              <details className="group">
                <summary className="text-[10px] text-slate-400 text-center cursor-pointer hover:text-slate-500 select-none">
                  Advanced: fix as a standalone voice_id (clone) ▸
                </summary>
                <div className="mt-2 space-y-1.5 pt-2 border-t border-slate-100">
                  <p className="text-[10px] text-amber-600/70 text-center">
                    <Warning size={12} weight="bold" className="inline" /> Cloning extracts voice characteristics from the preview audio; the resulting voice may differ slightly from the mixed preview. Only use this when you need a fixed voice_id.
                  </p>
                  <button onClick={handleBakeVoice} disabled={isBaking}
                    ...
                    <Lock size={14} weight="bold" /> {isBaking ? 'Fixing...' : 'Fix Voice (Clone)'}
                  </button>
                </div>
              </details>
            </div>
        ...
        <div className="bg-slate-50 rounded-2xl p-3 text-[10px] text-slate-400 space-y-1">
          <div>Voice strands: {timberWeights.filter(tw => tw.voice_id.trim()).length}</div>
          {timberWeights.filter(tw => tw.voice_id.trim()).length > 1 && (
            <div>Recipe: {timberWeights.filter(tw => tw.voice_id.trim()).map(tw => `${tw.voice_id}×${tw.weight}`).join(' + ')}</div>
          )}
          {(modifyPitch !== 0 || modifyIntensity !== 0 || modifyTimbre !== 0) && (
            <div>Modifiers: pitch={modifyPitch} intensity={modifyIntensity} timbre={modifyTimbre}</div>
          )}
          {soundEffect && <div>Sound effect: {soundEffect}</div>}
          {emotion && <div>Emotion: {emotion}</div>}
          <div>Parameters: {minimaxParamVersion === 'natural-v2' ? 'New Natural Parameters' : 'Classic Parameters'}</div>
        </div>
        ...
        <div className="absolute inset-0 z-50 bg-black/40 flex items-end">
          <div className="bg-white w-full rounded-t-3xl max-h-[70%] flex flex-col animate-slide-up">
            <div className="p-4 border-b border-slate-100 flex items-center justify-between shrink-0">
              <span className="text-sm font-bold text-slate-700">Select Voice</span>
              <button onClick={() => setShowVoicePicker(false)} className="text-xs px-3 py-1 bg-slate-100 rounded-full">Close</button>
            </div>
            <div className="px-4 pt-3 shrink-0">
              <input value={voiceSearch} onChange={e => setVoiceSearch(e.target.value)}
                className="w-full bg-slate-50 rounded-xl px-3 py-2 text-xs border border-slate-200"
                placeholder="Search by voice name or ID..." />
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-1">
              {filteredVoices.map(v => (
                <button key={v.voice_id} onClick={() => {
                  ...
                  <div className="font-medium text-slate-700 truncate">{v.voice_name || 'Unnamed'}</div>
                  <div className="text-[10px] text-slate-400 truncate">{v.voice_id}</div>
                </button>
              ))}
              {filteredVoices.length === 0 && (
                <div className="text-center py-8 text-slate-300 text-xs">
                  {availableVoices.length === 0 ? 'Please load the voice library first' : 'No matching voices found'}
                </div>
              )}
            </div>
          </div>
