...
const WorldbookApp: React.FC = () => {
    ...
    const groupedBooks = useMemo(() => {
        ...
        const defaultCat = 'Uncategorized (General)';
    ...
    const handleCreate = () => {
        ...
        setIsEditing(true);
        trackEvent('Open Worldbook Editor', { mode: 'create' });
    };
    ...
    const handleEdit = (book: Worldbook) => {
        ...
        setShowCategoryPicker(false);
        setIsEditing(true);
        trackEvent('Open Worldbook Editor', { mode: 'edit' });
    };
    ...
    const handleSave = async () => {
        ...
        const category = tempCategory.trim() || 'Uncategorized (General)';
        ...
        if (!tempConstant && primaryKeywords.length === 0) {
            addToast('Keyword trigger mode requires at least one primary keyword', 'error');
            return;
        }
        const entryConfig = {
            disable: !tempEnabled,
        ...
        if (editingBook) {
            ...
            addToast('Saved (synced to related characters)', 'success');
        } else {
    ...
    const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const category = file.name.replace(/\.json$/i, '').trim() || 'Imported Worldbook';
            const imported = parseStandardWorldbook(text, category);
            for (const book of imported) await addWorldbook(book);
            addToast(`Imported ${imported.length} worldbook entries`, 'success');
            setExpandedCategory(category);
        } catch (error: any) {
            addToast(error?.message || 'Worldbook import failed', 'error');
        } finally {
    ...
    const handleExportGroup = async (event: React.MouseEvent, category: string, books: Worldbook[]) => {
        ...
        const result = await shareOrDownloadFile({
            ...
            shareTitle: `Export Worldbook "${category}"`,
        });
        const verb = result === 'shared' ? 'Opened share for' : 'Exported';
        addToast(`${verb} "${category}" — ${books.length} entries`, 'success');
        trackEvent('Export Group as Standard Worldbook');
    };
    ...
    const confirmBulkDelete = async () => {
        ...
        leaveSelectionMode();
        addToast(`Deleted ${ids.length} worldbook entries`, 'success');
    };
            ...
            <div className="h-full w-full bg-[#f5f6fa] flex flex-col font-sans animate-fade-in">
                <div className="bg-white/90 backdrop-blur-xl border-b border-slate-200/70 shrink-0 z-20" style={{ paddingTop: 'var(--safe-top)' }}>
                    <div className="h-16 max-w-2xl mx-auto w-full flex items-center justify-between px-5">
                        <button onClick={() => setIsEditing(false)} className="px-3 py-2 -ml-3 rounded-xl text-slate-500 font-semibold text-sm hover:bg-slate-100 active:scale-95 transition-all">Cancel</button>
                        <div className="text-center">
                            <div className="text-[10px] font-bold tracking-[0.16em] text-indigo-400 uppercase">Worldbook</div>
                            <div className="text-sm font-bold text-slate-800 mt-0.5">{editingBook ? 'Edit Entry' : 'New Entry'}</div>
                        </div>
                        <button onClick={handleSave} className="px-4 py-2 -mr-1 bg-indigo-500 text-white rounded-xl text-xs font-bold shadow-sm shadow-indigo-200 active:scale-95 transition-all hover:bg-indigo-600">Save</button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto">
                    <div className="w-full max-w-2xl mx-auto px-5 py-5 pb-10 space-y-4">
                        <div className="bg-white rounded-[1.5rem] border border-slate-200/70 p-5 shadow-sm shadow-slate-200/40 space-y-4">
                            <div>
                                <div className="text-[11px] font-bold tracking-[0.14em] text-indigo-500 uppercase">Basic Info</div>
                                <p className="text-[10px] text-slate-400 mt-1">Used to identify, organize, and attach this worldbook entry.</p>
                            </div>
                            <div>
                            <label className="text-xs font-bold text-slate-500 mb-2 block">Title</label>
                            <input
                                value={tempTitle}
                                onChange={e => setTempTitle(e.target.value)}
                                placeholder="e.g. Magic system, company background..."
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="e.g. World setting, characters, geography..."
                                ...
                                {categoryNames.length > 0 && (
                                    <div className="mt-2">
                                        ...
                                        >
                                            ...
                                            Choose an existing category ({categoryNames.length})
                                        </button>
                                        {showCategoryPicker && (
                                            <div className="mt-1.5 max-h-36 overflow-y-auto rounded-xl border border-slate-200 bg-slate-50/60 p-2 flex flex-wrap gap-1.5 overscroll-contain">
                                                {filteredCategorySuggestions.length === 0 ? (
                                                    <span className="text-[10px] text-slate-400 px-1 py-0.5">No category matches "{tempCategory.trim()}" — a new one will be created on save.</span>
                                                ) : (
                                ...
                                <p className="text-[10px] text-slate-400 mt-1.5 px-1">Entries with the same category name are grouped automatically; type to filter the suggestions above.</p>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-[1.5rem] border border-slate-200/70 p-5 shadow-sm shadow-slate-200/40 space-y-5">
                            <div className="flex items-center justify-between gap-4">
                                <div>
                                    <div className="text-xs font-bold text-slate-700">Enable Entry</div>
                                    <p className="text-[10px] text-slate-400 mt-1">When off, the content is kept but not injected into the prompt.</p>
                                </div>
                                <button
                                    type="button"
                            ...
                            <div className="border-t border-slate-100 pt-4">
                                <label className="text-[11px] font-bold text-slate-400 uppercase mb-2 block tracking-[0.12em]">Trigger Method</label>
                                <label className="flex items-center gap-3 py-2 cursor-pointer select-none">
                                    ...
                                    />
                                    <span className="text-sm font-semibold text-slate-700">Enable Keyword Trigger</span>
                                </label>
                                <p className={`text-[10px] leading-relaxed mt-1 pl-7 ${tempConstant ? 'text-slate-400' : 'text-indigo-500'}`}>
                                    {tempConstant
                                        ? 'Unchecked: keywords are not checked, this worldbook entry is always active.'
                                        : 'Checked: only active when a primary keyword matches; a keyword is required to save.'}
                                </p>
                            </div>
                            ...
                            {!tempConstant && (
                                <div className="space-y-4 animate-fade-in">
                                    <div>
                                        <label className="text-xs font-bold text-slate-400 mb-2 block">Primary Keywords</label>
                                        <input
                                            value={tempKeywords}
                                            onChange={e => setTempKeywords(e.target.value)}
                                            placeholder="Separate multiple keywords with commas or line breaks"
                                            className="w-full text-sm text-slate-700 bg-slate-50/80 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-50 transition-all"
                                        />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-slate-400 mb-2 block">Secondary Keywords (optional)</label>
                                        <input
                                            value={tempSecondaryKeywords}
                                            onChange={e => setTempSecondaryKeywords(e.target.value)}
                                            placeholder="Used to further narrow the trigger condition"
                                            className="w-full text-sm text-slate-700 bg-slate-50/80 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-50 transition-all"
                                        />
                                    </div>
                                    {splitWorldbookKeywords(tempSecondaryKeywords).length > 0 && (
                                        <div>
                                            <label className="text-xs font-bold text-slate-400 mb-2 block">Secondary Keyword Condition</label>
                                            <select
                                                value={tempSelectiveLogic}
                                                onChange={e => setTempSelectiveLogic(Number(e.target.value) as WorldbookSelectiveLogic)}
                                                className="w-full text-sm text-slate-700 bg-slate-50/80 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-50 transition-all"
                                            >
                                                <option value={0}>Match at least one</option>
                                                <option value={3}>Match all</option>
                                                <option value={2}>Match none</option>
                                                <option value={1}>Not all match</option>
                                            </select>
                                        </div>
                                    )}
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-slate-400 mb-2 block">Recent Messages to Scan</label>
                                        <input
                                            type="number"
                                            min={0}
                                            value={tempScanDepth}
                        ...
                        <div className="bg-white rounded-[1.5rem] border border-slate-200/70 p-5 shadow-sm shadow-slate-200/40 space-y-4">
                            <div>
                                <div className="text-[11px] font-bold tracking-[0.14em] text-indigo-500 uppercase">Injection Settings</div>
                                <p className="text-[10px] text-slate-400 mt-1">Controls the entry's position and priority in the prompt.</p>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 mb-2 block">Injection Position</label>
                                <select
                                    value={tempPosition}
                                    onChange={e => {
                                        const nextPosition = Number(e.target.value) as WorldbookPosition;
                                        setTempPosition(nextPosition);
                                        trackEvent('Switch Worldbook Injection Position', { position: nextPosition });
                                    }}
                                    className="w-full text-sm text-slate-700 bg-slate-50/80 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-50 transition-all"
                                >
                                    {(Object.entries(WORLDBOOK_POSITION_LABELS) as [string, string][]).map(([value, label]) => (
                                        <option key={value} value={value}>
                                            {label}{value === '1' ? ' (Default · legacy position)' : ''}
                                        </option>
                                    ))}
                                </select>
                            ...
                            {tempPosition === 4 && (
                                <div className="grid grid-cols-2 gap-3 animate-fade-in">
                                    <div>
                                        <label className="text-xs font-bold text-slate-400 mb-2 block">Message Depth</label>
                                        <input
                                            type="number"
                                            min={0}
                                            value={tempDepth}
                                            ...
                                            className="w-full text-sm text-slate-700 bg-slate-50/80 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-50 transition-all"
                                        />
                                        <p className="text-[10px] text-slate-400 mt-1 px-1">0 is closest to the newest message; higher numbers go further back.</p>
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-slate-400 mb-2 block">Message Role</label>
                                        <select
                                            value={tempRole}
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="text-xs font-bold text-slate-400 mb-2 block">Insertion Order</label>
                                    <input
                                        type="number"
                                        value={tempOrder}
                            ...
                            <div className="rounded-xl border border-indigo-100 bg-indigo-50/70 px-4 py-3 text-[10px] leading-relaxed text-indigo-700">
                                <span className="font-bold">Leaving "Enable Random Probability" unchecked doesn't mean the entry is inactive.</span>
                                When unchecked, the random check is skipped entirely: as long as the entry is enabled and its constant/keyword condition is met, it passes at 100%. When checked, it re-rolls against the percentage above whenever the condition is met.
                            </div>
                        </div>

                        <div className="bg-white rounded-[1.5rem] border border-slate-200/70 p-5 shadow-sm shadow-slate-200/40">
                            <div className="text-[11px] font-bold tracking-[0.14em] text-indigo-500 uppercase">Content</div>
                            <p className="text-[10px] text-slate-400 mt-1 mb-3">Markdown supported; only enter the content that actually needs to be injected into the model.</p>
                            <textarea
                                value={tempContent}
                                onChange={e => setTempContent(e.target.value)}
                                placeholder="Enter detailed content here, Markdown formatting supported..."
    ...
    return (
        <div className="h-full w-full relative overflow-hidden font-sans bg-slate-100 flex flex-col">
                ...
                <div className="flex items-center px-6 py-3">
                    <div className="flex justify-between items-center w-full">
                        ...
                        <span className="font-bold text-slate-700 text-lg tracking-wide flex items-center gap-2">
                            <DiamondsFour size={18} className="text-indigo-500" /> Worldbook
                        </span>
                            ...
                            {worldbooks.length > 0 && (
                                <button
                                    onClick={() => {
                                        ...
                                        setIsSelecting(true);
                                        trackEvent('Enter Bulk Management Mode');
                                    }}
                                    ...
                                    title={isSelecting ? 'Exit Bulk Management' : 'Manage Worldbook Entries'}
                                >
                                    {isSelecting ? <X size={15} weight="bold" /> : <Check size={15} weight="bold" />}
                                    {isSelecting ? 'Cancel' : 'Manage'}
                                </button>
                            )}
                            <input ref={importRef} type="file" className="hidden" onChange={handleImport} />
                            <button
                                onClick={() => { setShowImportConfirm(true); trackEvent('Open Import Worldbook Dialog'); }}
                                ...
                                title="Import Standard Worldbook"
                            >
            ...
            {isSelecting && (
                <div className="relative z-10 shrink-0 bg-white/90 backdrop-blur-xl border-b border-indigo-100 px-5 py-2.5 flex items-center gap-3 shadow-sm">
                    ...
                    >
                        ...
                        {selectedBookIds.size === worldbooks.length ? 'Deselect All' : 'Select All'}
                    </button>
                    <span className="text-[11px] text-slate-400">{selectedBookIds.size} / {worldbooks.length} selected</span>
                    <button
            ...
            <div className="flex-1 overflow-y-auto p-5 pb-24 space-y-4 no-scrollbar relative z-0">
                <div className="rounded-2xl border border-indigo-100/80 bg-white/75 backdrop-blur-md p-4 shadow-sm text-slate-600">
                    <div className="flex items-center gap-2 text-xs font-bold text-indigo-600">
                        <BookOpen size={16} weight="bold" /> What does a worldbook do?
                    </div>
                    <p className="mt-2 text-[11px] leading-relaxed">
                        A worldbook is a set of supplemental settings conditionally provided to the AI — useful for world lore, character relationships, locations, rules, and more. It doesn't send messages on its own and isn't the same as character memory.
                    </p>
                    <p className="mt-1.5 text-[11px] leading-relaxed text-slate-500">
                        After creating or importing, it still needs to be attached under "Extended Settings" on the character edit page; when generating a chat reply, only enabled entries that satisfy their constant or keyword condition (plus an optional probability check) get injected into the prompt.
                    </p>
                    <p className="mt-2 rounded-xl bg-indigo-50 px-3 py-2 text-[10px] leading-relaxed text-indigo-700">
                        Note: leaving "Enable Random Probability" off does not mean random selection — the condition passes at 100% when met; it does not mean the entry is "inactive".
                    </p>
                </div>
                ...
                {Object.keys(groupedBooks).length === 0 && (
                    <div className="flex flex-col items-center justify-center h-64 text-slate-400 gap-4 opacity-60">
                        ...
                        <span className="text-xs font-medium">The world is still blank...</span>
                    </div>
                )}
                    ...
                    <div key={category} className="animate-slide-up">
                        ...
                        >
                            ...
                            <button
                                ...
                                title="Export this group as a standard worldbook"
                            >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex flex-wrap gap-1.5 mt-2 pl-3.5">
                                                <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold ${book.disable ? 'bg-slate-200 text-slate-500' : 'bg-indigo-50 text-indigo-500'}`}>
                                                    {book.disable ? 'Disabled' : (book.constant ?? !(book.key && book.key.length > 0)) ? 'Constant' : 'Keyword'}
                                                </span>
                                    ...
                                    {previewBookId === book.id && (
                                        <div className="px-4 pb-4 pt-0 animate-fade-in">
                                            ...
                                            <p className="text-xs text-slate-600 leading-relaxed whitespace-pre-wrap font-light select-text">
                                                {book.content || <span className="italic text-slate-400">No content yet...</span>}
                                            </p>
                                        </div>
                                    )}
                                </div>
            ...
            >
                <div className="py-3 text-sm text-slate-600 flex flex-col items-center gap-4">
                    ...
                    <p className="text-center leading-6">
                        Please note: if this isn't your own work, make sure the worldbook's author allows it to be used in this free virtual phone app.
                    </p>
                </div>
            </Modal>
            ...
            <Modal
                ...
                footer={
                    <div className="flex gap-3 w-full">
                        <button onClick={() => setShowBulkDeleteConfirm(false)} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-2xl active:scale-95 transition-transform">Cancel</button>
                        <button onClick={confirmBulkDelete} className="flex-1 py-3 bg-red-500 text-white font-bold rounded-2xl shadow-lg shadow-red-200 active:scale-95 transition-transform">Delete {selectedBookIds.size}</button>
                    </div>
                }
            >
                <div className="text-center py-4 text-sm text-slate-600 flex flex-col items-center gap-3">
                    ...
                    <div>
                        Are you sure you want to delete the selected <span className="font-bold text-slate-900">{selectedBookIds.size}</span> worldbook entries?
                        <br/><span className="text-xs text-red-400 opacity-80 mt-1 block">This will also remove them from any attached characters, and can't be undone.</span>
                    </div>
                </div>
            </Modal>
            ...
            <Modal
                ...
                footer={
                    <div className="flex gap-3 w-full">
                        <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-2xl active:scale-95 transition-transform">Cancel</button>
                        <button onClick={confirmDelete} className="flex-1 py-3 bg-red-500 text-white font-bold rounded-2xl shadow-lg shadow-red-200 active:scale-95 transition-transform">Confirm Delete</button>
                    </div>
                }
            >
                <div className="text-center py-4 text-sm text-slate-600 flex flex-col items-center gap-3">
                    ...
                    <div>
                        Are you sure you want to delete <span className="font-bold text-slate-900">"{editingBook?.title}"</span>?
                        <br/><span className="text-xs text-red-400 opacity-80 mt-1 block">This can't be undone.</span>
                    </div>
                </div>
            </Modal>
        </div>
