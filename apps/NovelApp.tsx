...
const NovelApp: React.FC = () => {
    ...
    const savePersonaDraft = () => {
        if (!libraryChar) return;
        if (!personaDraft.trim()) { addToast('Profile content cannot be empty', 'error'); return; }
        ...
        trackEvent('Save Character Writing Profile');
    };
    ...
    const handleCreateBook = () => {
        if (!tempTitle.trim()) { addToast('Please enter a title', 'error'); return; }
        ...
        setView('write');
        resetTempState();
        trackEvent('Create a New Manuscript');
    };
    ...
    const handleEditBookSettings = () => {
        ...
        setView('settings');
        trackEvent('Open Novel Settings Page');
    };
    ...
    const handleSaveSettings = async () => {
        ...
        setView('write');
        addToast('Settings updated, content intact', 'success');
    };
    ...
    const handleDeleteBook = async (id: string) => {
        setConfirmDialog({
            isOpen: true, title: 'Delete Work', message: 'Are you sure you want to delete this novel? This cannot be undone.', variant: 'danger',
            onConfirm: () => { deleteNovel(id); if (activeBook?.id === id) setView('shelf'); addToast('Deleted', 'success'); setConfirmDialog(null); trackEvent('Delete a Manuscript'); }
        });
    };
    ...
    const handleCoverUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            try {
                ...
                trackEvent('Upload Manuscript Cover Image');
            } catch (e) { addToast('Image processing failed', 'error'); }
        }
    };
    ...
    const openProtagonistEdit = (p?: NovelProtagonist) => {
        setEditingProtagonist(p || { id: `proto-${Date.now()}`, name: '', role: 'Protagonist', description: '' });
        setIsProtagonistModalOpen(true);
    };

    const saveProtagonist = () => {
        if (!editingProtagonist || !editingProtagonist.name.trim()) { addToast('Character name cannot be empty', 'error'); return; }
        ...
        trackEvent('Save a Story Character');
    };

    const handleImportProtagonist = (p: {name: string, role?: string, description: string}) => {
        const newP: NovelProtagonist = { id: `proto-${Date.now()}-${Math.random()}`, name: p.name, role: p.role || 'Protagonist', description: p.description || '' };
        setTempProtagonists(prev => [...prev, newP]);
        setIsProtoImportOpen(false);
        addToast(`Imported character: ${p.name}`, 'success');
        trackEvent('Import a Story Character', { source: importTab });
    };
    ...
    const importWorldbook = (wb: any) => {
        ...
        addToast(`Imported setting: ${wb.title}`, 'success');
        trackEvent('Import a Worldbook Setting');
    };
    ...
    const ProtagonistCard = ({ p, onDelete, onClick }: { p: NovelProtagonist, onDelete?: () => void, onClick?: () => void }) => (
        <div onClick={onClick} className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm relative group cursor-pointer hover:border-slate-400 transition-colors">
            ...
            <div className="text-xs text-slate-500 mt-1 line-clamp-2">{p.description || "No description yet"}</div>
            ...
            <div className="h-full w-full bg-slate-50 flex flex-col font-sans">
                <div className="bg-white/80 backdrop-blur-md border-b border-slate-200 shrink-0 sticky top-0 z-20" style={{ paddingTop: 'var(--chrome-top)' }}>
                    ...
                    <div className="flex justify-between items-center w-full">
                        ...
                        <span className="font-bold text-slate-800 text-lg tracking-wide">Character Library</span>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar">
                    <section>
                        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2"><Robot size={14} /> System Characters (AI Collaborators)</h3>
                        {/* 分组筛选（没建分组时不渲染） */}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {filterCharactersByGroup(characters, characterGroups, libraryGroupId).map(c => (
                                <div key={c.id} onClick={() => { setLibraryPersonaChar(c); setShowPersonaModal(true); }} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center gap-3 cursor-pointer hover:shadow-md transition-all active:scale-95">
                                    ...
                                    <div className="text-center"><div className="font-bold text-slate-700 text-sm">{c.name}</div><div className="text-[10px] text-slate-400 mt-1 px-2 py-0.5 bg-slate-50 rounded-full">Collaborator</div></div>
                                </div>
                            ))}
                        </div>
                    </section>
                    <section>
                        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2"><MaskHappy size={14} /> Story Characters from History</h3>
                        {historyProtagonists.length === 0 ? <div className="text-center py-8 text-slate-400 text-xs">No historical character data yet</div> : <div className="grid grid-cols-1 gap-3">{historyProtagonists.map((p, idx) => (<div key={idx} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm"><div className="flex justify-between items-start mb-2"><span className="font-bold text-slate-800">{p.name}</span><span className="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded border border-indigo-100">{p.role}</span></div><p className="text-xs text-slate-500 leading-relaxed line-clamp-3">{p.description || "No description yet"}</p></div>))}</div>}
                    </section>
                </div>

                <Modal isOpen={showPersonaModal} title={libraryChar?.name || 'Character Style'} onClose={closePersonaModal} footer={
                    isEditingPersona ? (<>
                        <button onClick={() => setIsEditingPersona(false)} className="flex-1 py-3 bg-slate-100 text-slate-500 font-bold rounded-2xl active:scale-95 transition-transform">Cancel</button>
                        <button onClick={savePersonaDraft} className="flex-1 py-3 bg-slate-800 text-white font-bold rounded-2xl active:scale-95 transition-transform">Save</button>
                    </>) : (<>
                        <button onClick={startEditPersona} className="flex-1 py-3 bg-indigo-50 text-indigo-600 font-bold rounded-2xl active:scale-95 transition-transform">Edit Profile</button>
                        <button onClick={closePersonaModal} className="flex-1 py-3 bg-slate-100 text-slate-500 font-bold rounded-2xl active:scale-95 transition-transform">Close</button>
                    </>)
                }>
                        ...
                        {libraryChar ? (
                            isEditingPersona ? (
                                <div className="space-y-2">
                                    ...
                                    <button onClick={() => { setPersonaDraft(analyzeWriterPersonaSimple(libraryChar)); trackEvent('Reset Writing Profile to Auto-Analysis'); }} className="text-xs text-slate-400 underline">Reset to auto-analysis</button>
                                </div>
                            ) : (
            ...
            <div className="h-full w-full bg-slate-50 flex flex-col font-sans relative">
                ...
                <div className="p-6 grid grid-cols-2 gap-5 overflow-y-auto pb-24">
                    {novels.map(book => {
                        ...
                        return (
                            <div key={book.id} onClick={() => { setActiveBook(book); setView('write'); trackEvent('Open Manuscript Writing Page'); }} className="group relative aspect-auto min-h-[14rem] bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-slate-100 cursor-pointer flex flex-col">
                                ...
                                <div className="p-4 flex-1 flex flex-col justify-between">
                                    <p className="text-xs text-slate-500 line-clamp-3 leading-relaxed mb-3">{book.summary || 'No synopsis yet...'}</p>
                                    <div className="flex items-center justify-between pt-3 border-t border-slate-50"><div className="flex -space-x-2">{characters.filter(c => book.collaboratorIds.includes(c.id)).map(c => (<TokenImg key={c.id} value={c.avatar} className="w-6 h-6 rounded-full border-2 border-white object-cover" />))}</div><span className="text-[10px] text-slate-400 font-mono bg-slate-50 px-2 py-0.5 rounded-full">{(wordCount/1000).toFixed(1)}k words</span></div>
                                </div>
                    ...
                    {novels.length === 0 && <div className="col-span-2 flex flex-col items-center justify-center h-64 text-slate-300 gap-3"><PenNib size={48} className="opacity-50" /><span className="text-sm font-sans">Tap the top-right to start writing</span></div>}
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="h-full w-full bg-slate-50 flex flex-col font-sans relative">
                ...
                <div className="bg-white border-b border-slate-200 shrink-0 sticky top-0 z-20" style={{ paddingTop: 'var(--chrome-top)' }}>
                    ...
                    <button onClick={() => setView(view === 'create' ? 'shelf' : 'write')} className="text-slate-500 text-sm">Cancel</button>
                    <span className="font-bold text-slate-800">{view === 'create' ? 'New Manuscript' : 'Novel Settings'}</span>
                    <button onClick={view === 'create' ? handleCreateBook : handleSaveSettings} className="bg-slate-800 text-white px-4 py-1.5 rounded-full text-xs font-bold shadow-md active:scale-95 transition-transform">Save</button>
                    </div>
                </div>
                <div className="flex-1 overflow-y-auto p-6 space-y-8 pb-20">
                    <section className="space-y-4">
                        <input value={tempTitle} onChange={e => setTempTitle(e.target.value)} placeholder="Title" className="w-full text-2xl font-bold bg-transparent border-b border-slate-200 py-2 outline-none focus:border-slate-800 font-serif" />
                        <input value={tempSubtitle} onChange={e => setTempSubtitle(e.target.value)} placeholder="Volume name/subtitle" className="w-full text-sm font-bold bg-transparent border-b border-slate-200 py-2 outline-none focus:border-slate-800 text-slate-600" />
                        <textarea value={tempSummary} onChange={e => setTempSummary(e.target.value)} placeholder="One-line synopsis..." className="w-full h-20 bg-slate-100 rounded-xl p-3 text-sm resize-none outline-none" />
                        <div>
                            <label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Page Style</label>
                        ...
                        <div>
                            <label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Custom Cover</label>
                            <div className="flex gap-3 items-center">
                                ...
                                <div className="flex-1 space-y-2"><input value={coverInputUrl} onChange={e => setCoverInputUrl(e.target.value)} onBlur={handleCoverUrlBlur} placeholder="Paste an image link..." className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs outline-none focus:border-slate-400" />{tempCoverImage && <button onClick={() => { setTempCoverImage(''); setCoverInputUrl(''); }} className="text-xs text-red-400 underline">Clear cover</button>}</div>
                            </div>
                        </div>
                    </section>
                    <section className="space-y-4">
                        <div className="flex justify-between items-center"><label className="text-xs font-bold text-slate-400 uppercase block">World Setting</label><button onClick={() => { setIsWorldbookModalOpen(true); trackEvent('Open Import Worldbook Dialog'); }} className="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-1 rounded font-bold hover:bg-indigo-100 flex items-center gap-1"><Books size={12} /> Import Worldbook</button></div>
                        <textarea value={tempWorld} onChange={e => setTempWorld(e.target.value)} placeholder="World setting..." className="w-full h-32 bg-white border border-slate-200 rounded-xl p-3 text-sm resize-none outline-none focus:border-slate-400" />
                    </section>
                    <section className="space-y-4">
                        <label className="text-xs font-bold text-slate-400 uppercase block">Collaborators</label>
                    ...
                    <section className="space-y-4">
                        <div className="flex justify-between items-center"><label className="text-xs font-bold text-slate-400 uppercase">Story Characters</label><div className="flex gap-2"><button onClick={() => { setIsProtoImportOpen(true); trackEvent('Open Import Story Character Dialog'); }} className="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-1 rounded font-bold hover:bg-indigo-100 border border-indigo-100 flex items-center gap-1"><FolderOpen size={12} /> Import</button><button onClick={() => openProtagonistEdit()} className="text-[10px] bg-slate-100 px-2 py-1 rounded text-slate-600 hover:bg-slate-200 transition-colors">+ Add</button></div></div>
                ...
                <Modal isOpen={isProtagonistModalOpen} title="Edit Character" onClose={() => setIsProtagonistModalOpen(false)} footer={<button onClick={saveProtagonist} className="w-full py-3 bg-slate-800 text-white font-bold rounded-2xl">Save</button>}>{editingProtagonist && (<div className="space-y-4"><div><label className="text-xs font-bold text-slate-400 uppercase block mb-1">Name</label><input value={editingProtagonist.name} onChange={e => setEditingProtagonist({...editingProtagonist, name: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold" /></div><div><label className="text-xs font-bold text-slate-400 uppercase block mb-1">Role</label><input value={editingProtagonist.role} onChange={e => setEditingProtagonist({...editingProtagonist, role: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm" placeholder="Protagonist / Antagonist" /></div><div><label className="text-xs font-bold text-slate-400 uppercase block mb-1">Description</label><textarea value={editingProtagonist.description} onChange={e => setEditingProtagonist({...editingProtagonist, description: e.target.value})} className="w-full h-32 bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm resize-none leading-relaxed" /></div></div>)}</Modal>
                <Modal isOpen={isProtoImportOpen} title="Import Character" onClose={() => setIsProtoImportOpen(false)}><div className="flex p-1 bg-slate-100 rounded-xl mb-3"><button onClick={() => setImportTab('system')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${importTab === 'system' ? 'bg-white shadow text-indigo-600' : 'text-slate-400'}`}>System Character (AI)</button><button onClick={() => setImportTab('history')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${importTab === 'history' ? 'bg-white shadow text-indigo-600' : 'text-slate-400'}`}>Historical Character</button></div><div className="max-h-[50vh] overflow-y-auto no-scrollbar space-y-3 p-1">{importTab === 'system' && characters.map(c => (<button key={c.id} onClick={() => handleImportProtagonist({name: c.name, role: 'Cameo', description: c.description})} className="w-full flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-xl hover:border-indigo-200 shadow-sm active:scale-95 transition-all text-left"><TokenImg value={c.avatar} className="w-8 h-8 rounded-full object-cover" /><div className="flex-1 min-w-0"><div className="font-bold text-sm text-slate-700">{c.name}</div><div className="text-[10px] text-slate-400 truncate">{c.description}</div></div></button>))}{importTab === 'history' && historyProtagonists.map((p, idx) => (<button key={`hist-${idx}`} onClick={() => handleImportProtagonist(p)} className="w-full flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-xl hover:border-indigo-200 shadow-sm active:scale-95 transition-all text-left"><div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-500 border border-slate-200">{p.name[0]}</div><div className="flex-1 min-w-0"><div className="font-bold text-sm text-slate-700">{p.name}</div><div className="text-[10px] text-slate-400 truncate">{p.role} - {p.description || "No description"}</div></div></button>))}</div></Modal>
                <Modal isOpen={isWorldbookModalOpen} title="Import Worldbook Setting" onClose={() => setIsWorldbookModalOpen(false)}><div className="max-h-[50vh] overflow-y-auto no-scrollbar space-y-2 p-1">{worldbooks.map(wb => (<button key={wb.id} onClick={() => importWorldbook(wb)} className="w-full text-left p-3 rounded-xl border border-slate-100 hover:border-indigo-300 bg-white shadow-sm active:scale-95 transition-all"><div className="font-bold text-slate-700 text-sm">{wb.title}</div><div className="text-[10px] text-slate-400 mt-1">{wb.category || 'Uncategorized'}</div></button>))}</div></Modal>
            </div>
        );
    }
