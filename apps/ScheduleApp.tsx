...
const ScheduleApp: React.FC = () => {
    ...
    const [supervisorGroupId, setSupervisorGroupId] = useState<string>(GROUP_FILTER_ALL); // Group filter for selecting the supervisor
    ...
    const [anniCharGroupId, setAnniCharGroupId] = useState<string>(GROUP_FILTER_ALL); // Group filter for the anniversary's linked character
    ...
    const toggleTheme = () => {
        ...
        trackEvent('Switch Schedule Theme', { theme: nextMode });
    };
    ...
    const generateTaskReward = async (task: Task) => {
        ...
        addToast(`${supervisor.name} is reviewing your progress...`, 'info');
        ...
        try {
            ...
            const userPrompt = `
### Scene: Task Completed
The user (${userProfile.name}) just completed a task/contract in real life: "${task.title}".
You are the supervisor.

### Task
Based on your persona, react to the user completing this task.
- If you're strict: grudgingly acknowledge it, or push them toward the next one.
- If you're gentle: give warm praise.
- If you're tsundere: express it awkwardly.
- **Key**: don't ask me what tone to use — decide **for yourself** based on your persona.
...
- No quotation marks.`;
            ...
            if (text) {
                ...
                await DB.saveMessage({
                    ...
                    role: 'system',
                    type: 'text',
                    content: `[System: ${userProfile.name} completed the task "${task.title}". ${supervisor.name} commented: "${text}"]`
                });
            } else {
                console.warn("AI returned empty content", data);
                addToast('Task completed (AI did not return a comment)', 'success');
            }
    ...
    const generateAnniversaryThought = async (anni: Anniversary) => {
        ...
        if (Date.now() - (anni.lastThoughtGeneratedAt || 0) > 10000) {
             addToast(`${char.name} is checking the calendar...`, 'info');
        }

        const daysDiff = getCalendarDayDifference(getLocalDateKey(), anni.date) ?? 0;
        const dayText = daysDiff > 0 ? `${daysDiff} days to go` : (daysDiff === 0 ? "It's today!" : `${Math.abs(daysDiff)} days ago`);
        ...
        <div className={`h-full w-full flex flex-col ${theme.font} ${theme.bg} ${theme.text} relative overflow-hidden transition-colors duration-500`}>
             ...
             <div className={`border-b ${theme.border} backdrop-blur-sm sticky top-0 z-20 shrink-0 relative transition-colors duration-300`} style={{ paddingTop: 'var(--safe-top)' }}>
                ...
                <div className="flex gap-2">
                    ...
                    {/* Add Button */}
                    <button onClick={() => { activeTab === 'quest' ? setShowTaskModal(true) : setShowAnniModal(true); trackEvent('Open New Entry Modal', { kind: activeTab }); }} className={`p-2 rounded-full active:scale-90 transition-transform ${theme.accent} ${currentThemeMode === 'minimal' ? 'shadow-[4px_4px_8px_#d1d9e6,-4px_-4px_8px_#ffffff]' : 'hover:bg-white/10'}`}>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-8 z-10">
                    ...
                    <div className={`w-full rounded-2xl p-5 relative overflow-hidden group transition-all duration-300 ${currentThemeMode === 'minimal' ? 'bg-[#eef2f6] shadow-[inset_5px_5px_10px_#d1d9e6,inset_-5px_-5px_10px_#ffffff]' : (currentThemeMode === 'soft' ? 'bg-gradient-to-r from-pink-300 to-purple-300 text-white shadow-lg shadow-pink-200' : 'bg-gradient-to-r from-slate-900 to-slate-800 border border-purple-500/30')}`}>
                        <div className="relative z-10">
                            <div className="flex justify-between items-start mb-2">
                                <div className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded ${currentThemeMode === 'minimal' ? 'text-slate-400' : 'text-white/80 bg-white/20'}`}>Coming Up</div>
                                <div className="text-3xl font-bold tracking-tighter">{getDaysUntil(upcomingAnni.date)} <span className="text-xs opacity-60 font-normal">days left</span></div>
                            </div>
                            ...
                            <div className={`flex items-start gap-3 p-3 rounded-xl ${currentThemeMode === 'minimal' ? 'bg-[#eef2f6] shadow-[5px_5px_10px_#d1d9e6,-5px_-5px_10px_#ffffff]' : 'bg-white/20 backdrop-blur-md'}`}>
                                ...
                                <div className={`text-xs font-medium leading-relaxed italic ${currentThemeMode === 'minimal' ? 'text-slate-500' : 'text-white/90'}`}>
                                    "{upcomingAnni.aiThought || "Loading..."}"
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {activeTab === 'quest' && (
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 mb-2 px-1">
                            ...
                            <h3 className={`text-xs font-bold uppercase tracking-[0.2em] ${theme.accent}`}>Active Tasks</h3>
                        </div>
                        ...
                        {tasks.filter(t => !t.isCompleted).length === 0 && (
                            <div className={`text-center py-12 border-2 border-dashed rounded-xl ${currentThemeMode === 'cyber' ? 'border-slate-800' : 'border-slate-200'}`}>
                                <div className={theme.textSub}>No tasks yet</div>
                            </div>
                        )}
                                ...
                                <div key={task.id} className={`${theme.card} p-4 flex items-center gap-4 group relative overflow-hidden transition-all duration-300`}>
                                    ...
                                    {isProcessing ? (
                                        <div className="flex items-center gap-2 px-2 py-2">
                                            ...
                                            <span className={`text-[10px] font-bold animate-pulse ${theme.accent}`}>Reviewing...</span>
                                        </div>
                                    ) : (
                                        <button
                        ...
                        {tasks.filter(t => t.isCompleted).length > 0 && (
                            <div className="pt-8 opacity-50">
                                <h3 className={`text-xs font-bold uppercase tracking-[0.2em] px-1 mb-4 ${theme.textSub}`}>Completed</h3>
                                {tasks.filter(t => t.isCompleted).map(task => (
                ...
                {activeTab === 'server_events' && (
                    <div className={`relative pl-6 space-y-8 before:absolute before:left-2 before:top-2 before:bottom-0 before:w-[1px] ${theme.decoLine}`}>
                        {/* Anniversaries List */}
                        <div>
                             <h3 className={`text-xs font-bold uppercase tracking-widest mb-6 -ml-6 pl-6 ${theme.textSub}`}>Timeline Events</h3>
                             <div className="space-y-4">
                                 {anniversaries.map(a => (
                        ...
                        {/* Completed Tasks History Log */}
                         <div>
                             <h3 className={`text-xs font-bold uppercase tracking-widest mb-6 -ml-6 pl-6 pt-4 ${theme.textSub}`}>Completion History</h3>
                             <div className="space-y-4">
                                 {tasks.filter(t => t.isCompleted).sort((a,b) => (b.completedAt || 0) - (a.completedAt || 0)).map(t => (
                                     <div key={t.id} className="relative">
                                         ...
                                         <div className={`text-xs ${theme.textSub} font-mono`}>[{new Date(t.completedAt || 0).toLocaleDateString()}] Task completed</div>
            ...
            {/* Task Modal */}
            <Modal isOpen={showTaskModal} title={currentThemeMode === 'cyber' ? "INITIALIZE QUEST" : "New Task"} onClose={() => setShowTaskModal(false)} footer={<button onClick={handleAddTask} className={`w-full py-3 font-bold transition-all ${theme.buttonPrimary}`}>Confirm</button>}>
                <div className={`space-y-6 ${currentThemeMode === 'minimal' ? 'p-2' : ''}`}>
                    <input autoFocus value={newTaskTitle} onChange={e => setNewTaskTitle(e.target.value)} placeholder="Task goal (e.g. Learn 10 new words)" className={`w-full px-4 py-3 text-sm focus:outline-none ${theme.input}`} />

                    <div>
                        <label className="text-[10px] font-bold text-slate-400 uppercase mb-2 block tracking-widest">Select Supervisor</label>
                        {/* Group filter (hidden if no groups exist). Modal is always on a white background, uses the light color scheme */}
            ...
            {/* Anniversary Modal */}
            <Modal isOpen={showAnniModal} title={currentThemeMode === 'cyber' ? "REGISTER EVENT" : "Add Anniversary"} onClose={() => setShowAnniModal(false)} footer={<button onClick={handleAddAnni} className={`w-full py-3 font-bold transition-all ${theme.buttonPrimary}`}>Save</button>}>
                <div className={`space-y-4 ${currentThemeMode === 'minimal' ? 'p-2' : ''}`}>
                    <input value={newAnniTitle} onChange={e => setNewAnniTitle(e.target.value)} placeholder="Event name (e.g. First time we met)" className={`w-full px-4 py-3 text-sm focus:outline-none ${theme.input}`} />
                    ...
                    <div>
                        <label className="text-[10px] font-bold text-slate-400 uppercase mb-2 block tracking-widest">Linked Character</label>
                        {/* Group filter (hidden if no groups exist). Modal is always on a white background, uses the light color scheme */}
                        <CharacterGroupFilterBar characters={characters} groups={characterGroups}
