...
const StoryVectorMemoryPanel: React.FC<Props> = ({ entry, onBack }) => {
    ...
    useEffect(() => {
        ...
        void listStoryVectorMemories(entry.id).then(rows => {
            ...
            if (active) addToastRef.current(error?.message || 'Failed to load story vector memory', 'error');
        }).finally(() => {
    ...
    const saveEdit = async () => {
        ...
        try {
            ...
            setEditing(null);
            addToast('Story memory and vector synced', 'success');
        } catch (error: any) {
            addToast(error?.message || 'Failed to edit story memory', 'error');
        } finally {
            setSaving(false);
        }
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            setDeleting(null);
            addToast(result.remoteDeleted === false ? 'Deleted locally; not yet synced remotely, please check your connection' : 'Deleted from this story\'s vector partition', result.remoteDeleted === false ? 'info' : 'success');
        } catch (error: any) {
            addToast(error?.message || 'Failed to delete story memory', 'error');
        } finally {
            setSaving(false);
        }
    };
        ...
        <main className='story-page-scroll flex-1 overflow-y-auto px-5 py-6 pb-24'>
            <div className='max-w-2xl mx-auto'>
                <section className='story-cinema-rule pb-6 border-b border-slate-200'>
                    ...
                    <h2 className='mt-4 text-2xl font-serif font-semibold'>This story's vector memory</h2>
                    <p className='mt-2 text-[11px] leading-5 text-slate-500'>There's only "{entry.title}"'s independent partition here, {nodes.length} entries in total. Long-press any entry to edit or delete it; actions here don't enter character memory and don't touch other stories.</p>
                </section>

                {loading ? <div className='py-20 grid place-items-center text-slate-400'><SpinnerGap size={24} className='animate-spin' /></div> : nodes.length === 0 ? <div className='py-16 text-center'><div className='text-sm font-semibold'>No vector archives yet</div><p className='mt-2 text-[10px] leading-5 text-slate-500'>Once the archive threshold is reached, older text will be distilled and written here.</p></div> : <>
