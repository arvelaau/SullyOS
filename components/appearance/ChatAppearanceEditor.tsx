...
const presets: Array<{ name: string; desc: string; config: Partial<OSTheme> }> = [
    {
        name: 'Default Chat',
        desc: 'Soft, universal chat shell',
        config: {
    ...
    {
        name: 'WeChat',
        desc: 'Flat, understated familiarity',
        config: {
    ...
    {
        name: 'Discord',
        desc: 'A more channel-like interface',
        config: {
            chatChromeStyle: 'floating',
    ...
    {
        name: 'iMessage',
        desc: 'Rounder, lighter character',
        config: {
            chatChromeStyle: 'soft',
            chatBackgroundStyle: 'mesh',
    ...
    },
    {
        ...
        desc: 'No avatars + edge-hugging + loose line spacing',
        config: {
            chatChromeStyle: 'flat',
            chatBackgroundStyle: 'plain',
            chatHeaderStyle: 'minimal',
    ...
    {
        name: 'Compact Chat',
        desc: 'Small tight text + top-aligned avatars',
        config: {
            chatChromeStyle: 'flat',
            chatBackgroundStyle: 'plain',
            chatHeaderStyle: 'default',
    ...
    {
        name: 'Pixel Terminal',
        desc: 'Faux-window style chat shell',
        config: {
...
const choices = {
    chrome: [
        { value: 'soft', label: 'Soft Mist', desc: 'Light glass feel' },
        { value: 'flat', label: 'Flat', desc: 'Cleaner and crisper' },
        { value: 'floating', label: 'Floating', desc: 'More pronounced layering' },
        { value: 'pixel', label: 'Pixel', desc: 'Hard-edged faux window' },
    ],
    background: [
        { value: 'plain', label: 'Plain' },
        { value: 'grid', label: 'Grid' },
        { value: 'paper', label: 'Paper' },
        { value: 'mesh', label: 'Ambient' },
    ],
    header: [
        { value: 'default', label: 'Default' },
        { value: 'minimal', label: 'Minimal' },
        { value: 'gradient', label: 'Gradient' },
        { value: 'wechat', label: 'WeChat-like' },
        { value: 'telegram', label: 'Telegram' },
        { value: 'discord', label: 'Discord' },
        { value: 'pixel', label: 'Pixel Window' },
    ],
    bubble: [
    ...
    status: [
        ...
        { value: 'pill', label: 'Status Pill' },
        { value: 'dot', label: 'Online Dot' },
    ],
    send: [
    ...
    avatarMode: [
        { value: 'grouped', label: 'Grouped', desc: 'Show avatar once per message group' },
        { value: 'every_message', label: 'Every Message', desc: 'Every message shows an avatar' },
    ],
    spacing: [
        { value: 'compact', label: 'Compact' },
        { value: 'default', label: 'Default' },
    ...
    timestamp: [
        ...
        { value: 'hover', label: 'On Hover (desktop)' },
        { value: 'never', label: 'Never Show' },
    ],
    emojiSize: [
        { value: 'small', label: 'Small', desc: '96px' },
        { value: 'medium', label: 'Medium', desc: '128px' },
        { value: 'large', label: 'Large', desc: '160px · legacy' },
    ],
} as const;
...
export const ChatAppearanceEditor: React.FC<Props> = ({ theme, updateTheme, onResetAllChrome, onOpenApp }) => {
    ...
    const PAGE_TITLES = ['Quick Presets', 'Chat Shell', 'Header', 'Bubbles & Avatars', 'Fine-Tuning', 'Stickers & Input Bar'];
    ...
    const previewMessages = [
        { id: 'ai-1', role: 'assistant', kind: 'text', text: 'This chat shell already looks a lot better than before.' },
        { id: 'ai-2', role: 'assistant', kind: 'voice', text: '00:08' },
        { id: 'user-1', role: 'user', kind: 'text', text: "Right, I'd like avatar frequency to be customizable too." },
        { id: 'user-2', role: 'user', kind: 'text', text: 'That way the different app vibes come through more clearly.' },
    ] as const;
        ...
        <div className="space-y-5">
            ...
            <section className="rounded-3xl border border-slate-100 bg-white p-3 shadow-sm">
                <div className="mb-2 flex items-baseline justify-between px-1">
                    <h2 className="text-[11px] font-bold uppercase tracking-widest text-slate-400">Live Preview</h2>
                    <span className="text-[9px] text-slate-300">Global setting · changes reflect immediately</span>
                </div>
                        ...
                        <div className={`flex items-center gap-3 ${headerAlign === 'center' ? 'justify-center text-center' : 'justify-between text-left'}`}>
                            <div className={`flex items-center gap-3 ${headerAlign === 'center' ? 'justify-center' : ''}`}>
                                ...
                                <div className={`sully-chat-status ${headerAlign === 'center' ? 'flex flex-col items-center' : ''}`}>
                                    <div className={`sully-chat-name text-xs font-bold ${headerTextClass}`}>Chat partner</div>
                        ...
                        {previewMessages.map((message, index) => {
                            ...
                            const bubbleNode = message.kind === 'voice' ? (
                                <div className={`sully-voice-bar flex min-w-[190px] items-center gap-2 rounded-2xl border px-3 py-2 ${headerStyle === 'discord' ? 'border-white/10 bg-white/10' : chromeStyle === 'pixel' ? 'border-2 border-[#8f674a] bg-[#fff7ed]' : 'border-black/5 bg-slate-100/90'}`}>
                                    ...
                                    <span className={`sully-voice-bar-toggle rounded-lg px-1.5 py-0.5 text-[8px] font-medium ${headerStyle === 'discord' ? 'bg-white/10 text-white/65' : chromeStyle === 'pixel' ? 'bg-[#eadfce] text-[#8f674a]' : 'bg-black/5 text-slate-500'}`}>To text</span>
                                </div>
                            ) : (
            ...
            <div className="rounded-2xl border border-dashed border-slate-200 bg-white/70 px-4 py-2.5 text-[10px] leading-relaxed text-slate-400">
                Chat shell settings live in the floating panel · tap the circle button on the right to collapse/expand, hold it to drag it out of the preview's way
            </div>
            ...
            <button
                ...
                aria-label={panelOpen ? 'Collapse chat shell settings panel' : 'Expand chat shell settings panel'}
            >
            ...
            >
                ...
                >
                {page === 0 && (<>
                    <p className="mb-3 text-[10px] text-slate-400">Swap the whole chat shell in one tap (avatars, bubbles, spacing, and fine-tuning included); switching presets clears any leftover fine-tuning first.</p>
                ...
                {page === 1 && (<>
                    <ChoiceGroup title="Chat Shell" items={choices.chrome} value={chromeStyle} onPick={(value) => updateTheme({ chatChromeStyle: value as OSTheme['chatChromeStyle'] })} />
                    <div className="mt-4">
                        <ChoiceGroup title="Message Area Background" items={choices.background} value={backgroundStyle} onPick={(value) => updateTheme({ chatBackgroundStyle: value as OSTheme['chatBackgroundStyle'] })} />
                    </div>
                </>)}

                {page === 2 && (<>
                <ChoiceGroup title="Header Style" items={choices.header} value={headerStyle} onPick={(value) => updateTheme({ chatHeaderStyle: value as OSTheme['chatHeaderStyle'] })} />
                <div className="mt-4">
                    <ChoiceGroup title="Header Alignment" items={choices.align} value={headerAlign} onPick={(value) => updateTheme({ chatHeaderAlign: value as OSTheme['chatHeaderAlign'] })} />
                </div>
                <div className="mt-4">
                    <ChoiceGroup title="Header Density" items={choices.density} value={headerDensity} onPick={(value) => updateTheme({ chatHeaderDensity: value as OSTheme['chatHeaderDensity'] })} />
                </div>
                <div className="mt-4">
                    <ChoiceGroup title="Online Status Style" items={choices.status} value={statusStyle} onPick={(value) => updateTheme({ chatStatusStyle: value as OSTheme['chatStatusStyle'] })} />
                </div>
                <div className="mt-4 flex items-center justify-between rounded-2xl bg-slate-50 px-3 py-2.5">
                    <div className="min-w-0 pr-3">
                        <div className="text-[11px] font-bold text-slate-700">Show mood bar</div>
                        <div className="mt-0.5 text-[10px] text-slate-400">The mood buff pills under the character's name; turning this off makes the header cleaner (position/style can also be tuned in "Whitebox Customization" via .sully-chat-buffs).</div>
                    </div>
                    <button
                ...
                {page === 3 && (<>
                <ChoiceGroup title="Message Bubble" items={choices.bubble} value={bubbleStyle} onPick={(value) => updateTheme({ chatBubbleStyle: value as OSTheme['chatBubbleStyle'] })} />
                <div className="mt-4">
                    <ChoiceGroup title="Avatar Shape" items={choices.avatarShape} value={avatarShape} onPick={(value) => updateTheme({ chatAvatarShape: value as OSTheme['chatAvatarShape'] })} />
                </div>
                <div className="mt-4">
                    <ChoiceGroup title="Avatar Size" items={choices.avatarSize} value={avatarSize} onPick={(value) => updateTheme({ chatAvatarSize: value as OSTheme['chatAvatarSize'] })} />
                </div>
                <div className="mt-4">
                    <ChoiceGroup title="Avatar Frequency" items={choices.avatarMode} value={avatarMode} onPick={(value) => updateTheme({ chatAvatarMode: value as OSTheme['chatAvatarMode'] })} />
                </div>
                <div className="mt-4">
                    <ChoiceGroup title="Message Density" items={choices.spacing} value={messageSpacing} onPick={(value) => updateTheme({ chatMessageSpacing: value as OSTheme['chatMessageSpacing'] })} />
                </div>
                <div className="mt-4">
                    <ChoiceGroup title="Timestamp" items={choices.timestamp} value={showTimestamp} onPick={(value) => updateTheme({ chatShowTimestamp: value as OSTheme['chatShowTimestamp'] })} />
                </div>
                <div className="mt-4 flex items-center justify-between rounded-2xl bg-slate-50 px-3 py-2.5">
                    <div className="min-w-0 pr-3">
                        <div className="text-[11px] font-bold text-slate-700">Sending-in-progress dots</div>
                        <div className="mt-0.5 text-[10px] text-slate-400">During Instant Push, three bouncing dots appear to the left of your own bubble.</div>
                    </div>
                    <button
                ...
                {page === 4 && (<>
                    <p className="mb-3 text-[10px] leading-relaxed text-slate-400">
                        Avatar visibility/alignment, edge-snapping, font size and line height — no more hand-writing CSS, changes reflect live in the preview above.
                        Users with hand-written styling code are unaffected: <b>your custom CSS takes priority</b> and will always override what's set here.
                    </p>
                    ...
                    <button
                        ...
                        Reset all fine-tuning to default (clears leftovers in one tap)
                    </button>
                    <p className="mt-2 text-[10px] text-slate-400">
                        What's set here is the global baseline. Want a separate look for one character? Go to their chat → the "+" menu → "Chat Styling".
                    </p>
                </>)}

                {page === 5 && (<>
                    <ChoiceGroup title="Sticker Size" items={choices.emojiSize} value={theme.chatEmojiSize || 'small'} onPick={(value) => updateTheme({ chatEmojiSize: value as OSTheme['chatEmojiSize'] })} />
                    <p className="mt-2 text-[10px] text-slate-400">The size of sticker images sent in chat and group chat. Any custom CSS that already tunes sticker size will continue to override this setting. (Sticker size can't be seen in the preview — go send one in a real chat to try it.)</p>
                    <div className="mt-4">
                        <ChoiceGroup title="Input Bar Style" items={choices.input} value={inputStyle} onPick={(value) => updateTheme({ chatInputStyle: value as OSTheme['chatInputStyle'] })} />
                    </div>
                    <div className="mt-4">
                        <ChoiceGroup title="Send Button" items={choices.send} value={sendButtonStyle} onPick={(value) => updateTheme({ chatSendButtonStyle: value as OSTheme['chatSendButtonStyle'] })} />
                    </div>
                </>)}
                </div>
            </section>
            ...
            <section className={groupClass}>
                <div className="mb-3">
                    <h2 className="text-sm font-bold uppercase tracking-widest text-slate-400">Global Default Notification Sound</h2>
                    <p className="mt-1 text-[10px] leading-relaxed text-slate-400">
                        When a character has no notification sound of their own, receiving a new message from them uses the default sound set here. A sound set individually in the character's "+" menu → "Notification Sound" overrides the global one.
                    </p>
                </div>
                <WhiteboxSoundEditor
                    ...
                    hint={<>🔔 <b>Global default</b>: when a character has no notification sound of their own, this plays when their latest message arrives. A sound the character sets individually overrides it.</>}
                />
            </section>
            ...
            <section className={groupClass}>
                <div className="mb-3 flex items-start justify-between gap-2">
                    <div>
                        <h2 className="text-sm font-bold uppercase tracking-widest text-slate-400">Advanced Styling · Where to Change What</h2>
                        <p className="mt-1 text-[10px] leading-relaxed text-slate-400">
                            This page controls the "overall shell" and fine-tuning; bubble appearance and deep customization each have their own dedicated place. Not sure which one takes effect? Tap the "?" on the right.
                        </p>
                    </div>
                    <button
                        ...
                        aria-label="Styling priority explanation"
                    >
                        ?
                    </button>
                </div>
                {showStyleHelp && (
                    <div className="mb-3 rounded-2xl bg-amber-50/70 border border-amber-200/60 px-4 py-3 text-[11px] text-slate-600 leading-relaxed space-y-2">
                        <p className="font-bold text-amber-700">"I changed it in all three places — which one actually applies?"</p>
                        <p>The three styling entry points each own their own area and usually don't conflict:</p>
                        <p>
                            <span className="font-semibold">🎛️ This page (visual settings)</span>: chat shell, avatars, spacing, fine-tuning. Use it to change the overall layout without writing a line of code.
                        </p>
                        <p>
                            <span className="font-semibold">🎨 Bubble Workshop</span>: the bubble's own look — color, corner radius, textures, decorations. Finished bubbles can be worn per character.
                        </p>
                        <p>
                            <span className="font-semibold">✍️ Whitebox Custom CSS</span>: hand-write code for deep customization (header, input bar, any part). Found in each character's chat → "+" → "Whitebox".
                        </p>
                        <p className="font-semibold">When the same thing is changed in more than one place, who wins:</p>
                        <p>
                            <span className="font-semibold text-amber-600">Visual settings &lt; Bubble theme &lt; Custom CSS</span>.
                            In other words: a Bubble Workshop theme overrides the settings on this page; your hand-written custom CSS has the highest priority and overrides both —
                            so styling code long-time users already have will never get broken by a toggle on this page.
                        </p>
                        <p>
                            If a character's settings "don't seem to apply", check in this order: first whether they have dedicated Whitebox CSS (chat "+" → "Whitebox"), then which bubble theme they're wearing (chat header conversation panel → Bubble Style), and only then this page.
                        </p>
                    </div>
                )}
                <div className="space-y-2">
                    <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-3 py-2.5">
                        <div className="min-w-0 pr-3">
                            <div className="text-[11px] font-bold text-slate-700">Want to change bubble color / texture / decoration</div>
                            <div className="mt-0.5 text-[10px] text-slate-400">Go craft one in the Bubble Workshop — once saved, it can be applied to a character directly.</div>
                        </div>
                        <button
                            ...
                            Go to Bubble Workshop →
                        </button>
                    </div>
                    <div className="rounded-2xl bg-slate-50 px-3 py-2.5">
                        <div className="text-[11px] font-bold text-slate-700">Want a separate fine-tuning setup for one character</div>
                        <div className="mt-0.5 text-[10px] text-slate-400">Go to their chat → the "+" menu → "Chat Styling" — you can override just details like font size and avatar, everything else follows the global setting.</div>
                    </div>
                    <div className="rounded-2xl bg-slate-50 px-3 py-2.5">
                        <div className="text-[11px] font-bold text-slate-700">Want to hand-write CSS for deep customization</div>
                        <div className="mt-0.5 text-[10px] text-slate-400">Go to the character's chat → the "+" menu → "Whitebox" — you can write and preview live there, and save/share presets too.</div>
                    </div>
                </div>
            </section>
            ...
            <section className={groupClass}>
                <div className="mb-3">
                    <h2 className="text-sm font-bold uppercase tracking-widest text-slate-400">Whitebox Customization (CSS)</h2>
                    <p className="mt-1 text-[10px] leading-relaxed text-slate-400">
                        Chat whitebox styling is now managed per character: go to that character's chat → "+" menu → "Whitebox" to configure, preview, and save presets.
                        If a character's CSS breaks and makes the chat UI unusable — even blocking settings — tap the one-tap restore-all button below to recover.
                    </p>
                </div>
                <button
                    onClick={() => { if (window.confirm('Restore all chat whitebox styling? This clears custom CSS for both the "global" and "per-character" scopes (other chat appearance settings are unaffected).')) onResetAllChrome?.(); }}
                    ...
                    Restore all chat whitebox styling (rescue)
                </button>
            </section>

            <div className="px-2 pb-2 text-center text-[10px] leading-relaxed text-slate-400">
                This version turns chat appearance into modular shell-swapping. If you want to go deeper later, we could split this into a separate chat shell per character, or even have different apps mimic different platform UIs.
            </div>
        </div>
    );
};
