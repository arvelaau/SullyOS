        ...
        <div className="fixed inset-0 z-[9998] flex items-center justify-center p-5 animate-fade-in">
            ...
            <div className="relative w-full max-w-sm bg-white/95 backdrop-blur-xl rounded-[2.5rem] shadow-2xl border border-pink-200/50 overflow-hidden animate-slide-up">
                ...
                <div className="pt-8 pb-4 px-6 text-center relative">
                    ...
                    <h2 className="text-lg font-extrabold text-slate-800">Sully seems to have something to tell you?</h2>
        ...
        <div className="fixed inset-0 z-[9998] flex items-center justify-center p-5 animate-fade-in">
            ...
            <div className="relative w-full max-w-sm bg-white/95 backdrop-blur-xl rounded-[2.5rem] shadow-2xl border border-white/30 overflow-hidden animate-slide-up max-h-[85vh] flex flex-col">
                <div className="px-6 pt-6 pb-2 text-center shrink-0">
                    ...
                    <p className="text-[11px] text-slate-400 mt-1">Once configured, you can view the Valentine's Day special push</p>
                </div>
...
export const ValentineSession: React.FC<ValentineSessionProps> = ({ charId, onClose }) => {
    ...
    const handleDeleteRecord = async (cId: string) => {
        try {
            ...
            addToast(`Deleted ${targetChar?.name || ''}'s Valentine's Day record`, 'success');
        } catch (e) {
    ...
    const generateValentineMessage = async (cId: string) => {
        ...
        if (!c) { setErrorMsg('Character not found'); setPhase('error'); return; }
    ...
    const handleExportImage = async () => {
        ...
        try {
            ...
            const blob = await new Promise<Blob>((resolve, reject) => {
                canvas.toBlob(result => result ? resolve(result) : reject(new Error('Failed to generate image')), 'image/png');
            });
            const result = await shareOrDownloadBlob({ blob, fileName, shareTitle: 'Special Moments - Export Image' });
        ...
        return (
            <div className="fixed inset-0 z-[9997] bg-gradient-to-b from-pink-50 via-white to-rose-50 flex flex-col animate-fade-in">
                ...
                <div className="h-16 flex items-center justify-between px-4 border-b border-pink-100 bg-white/80 backdrop-blur-sm shrink-0"
                    ...
                    <span className="font-bold text-slate-700">Special Moments - Valentine's Day</span>
                    <div className="w-8" />
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6" style={{ paddingBottom: 'calc(1.5rem + var(--safe-bottom))' }}>
                    <div className="text-center mb-6">
                        ...
                        <h2 className="text-lg font-bold text-slate-700">Choose who you want to hear from</h2>
                        <p className="text-xs text-slate-400 mt-1">Pick a character to hear their special Valentine's Day words</p>
                    </div>
                    ...
                    <p className="text-center text-[10px] text-slate-300 mt-4">Long-press a character to delete their Valentine's Day record</p>
                </div>
                    ...
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 animate-fade-in">
                        ...
                        <div className="relative bg-white rounded-3xl p-6 max-w-xs w-full shadow-2xl">
                            <div className="text-center mb-4">
                                ...
                                <h3 className="font-bold text-slate-700 text-base">Delete Valentine's Day Record</h3>
                                <p className="text-xs text-slate-400 mt-2 leading-relaxed">
                                    This will delete <span className="font-bold text-slate-600">{characters.find(c => c.id === deleteTargetId)?.name}</span>'s Valentine's Day record, including stored memories and the corresponding chat messages. This action cannot be undone.
                                </p>
                            </div>
                ...
                <div className="flex flex-col items-center gap-6">
                    ...
                    <div className="text-center">
                        <p className="text-pink-300 text-sm font-light tracking-widest animate-pulse">
                            {char?.name} is preparing what to say to you...
                        </p>
        ...
        return (
            <div className="fixed inset-0 z-[9997] bg-black flex flex-col items-center justify-center animate-fade-in p-8">
                <div className="text-center max-w-sm">
                    ...
                    <h2 className="text-white text-lg font-bold mb-2">Unable to connect right now</h2>
                ...
                <div className="absolute top-28 right-4 z-50 control-zone animate-fade-in" onClick={(e) => e.stopPropagation()}>
                    <div className="bg-black/70 backdrop-blur-xl rounded-2xl border border-white/15 p-4 w-56 shadow-2xl">
                        <div className="flex items-center justify-between mb-3">
                            <span className="text-[11px] font-bold text-white/80 tracking-wide">Sprite Adjustment</span>
                            <button onClick={(e) => { e.stopPropagation(); handleSaveSpriteConfig(); }} className="text-[10px] text-pink-400 font-bold">Done</button>
                        </div>
                        <div className="space-y-3">
                            ...
                            <div>
                                <div className="flex justify-between mb-1">
                                    <span className="text-[10px] text-white/50">Horizontal Position</span>
                            ...
                            <div>
                                <div className="flex justify-between mb-1">
                                    <span className="text-[10px] text-white/50">Vertical Position</span>
        ...
        <div className="mb-6" style={{ contain: 'layout paint' }}>
            ...
            >
                ...
                <div className="relative">
                    ...
                    {characters.length === 0 ? (
                        <div className="text-center text-xs text-white/70 py-3">No characters yet</div>
                    ) : (
                    ...
                    {characters.length > 0 && (
                        <p className={`text-[10px] ${theme.helpColor} mt-3 text-center`}>{footerNote || 'Long-press a character to delete the record'}</p>
                    )}
                </div>
            </div>
        </div>
...
export const SpecialMomentsApp: React.FC = () => {
    ...
    const handleL520DeleteRecord = async (cId: string) => {
        try {
            ...
            addToast(`Deleted ${targetChar?.name || ''}'s 520 record`, 'success');
        } catch {
            addToast('Delete failed', 'error');
        } finally {
    ...
    const handleWdDeleteRecord = async (cId: string) => {
        try {
            ...
            addToast(`Deleted ${targetChar?.name || ''}'s White Day record`, 'success');
        } catch {
            addToast('Delete failed', 'error');
        } finally {
    ...
    const handleQixiReturnToChat = async ({ message, card, replaySnapshot }: QixiReturnPayload) => {
        ...
        try {
            ...
            addToast('Failed to save Qixi message, please try again', 'error');
            throw error;
        }
    };
        ...
        return (
            <QixiDemoSession
                ...
                onPortraitConfigSave={(spriteConfig) => {
                    ...
                    addToast('Qixi sprite position saved, synced to Date mode', 'success');
                }}
        ...
        <div className="h-full w-full bg-gradient-to-b from-pink-50 via-white to-rose-50 flex flex-col font-light">
            ...
            <div className="border-b border-pink-100 bg-white/80 backdrop-blur-sm shrink-0" style={{ paddingTop: 'var(--safe-top)' }}>
                <div className="h-16 flex items-center justify-between px-4">
                    ...
                    <span className="font-bold text-slate-700">Special Moments</span>
                    <div className="w-8" />
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
                ...
                <SpecialEventCard
                    theme={THEME_QIXI}
                    icon="✦"
                    eyebrow="QIXI FESTIVAL · DEMO"
                    title="Starmoon Dream Fairytale"
                    subtitleActive="2026 Qixi — Fall into a context rift, searching for the one who's searching for you too"
                    subtitlePast="2026 Qixi — Relive that starmoon dream"
                    hintActive="Pick a character to jump straight into the story from the Starmoon homepage"
                    hintPast="Tap a character to relive the record"
                    isPast={false}
                    characters={characters}
                    recordKey={QIXI_DEMO_RECORD_KEY}
                    onPick={pickQixiCharacter}
                    onLongPressDelete={() => undefined}
                    footerNote="UI / CSS / Hand-drawn SVG · Fixed-coordinate star map · 1 LLM call on first run to generate memory star-lines"
                />
                ...
                {visibility.like520.show && (
                    <SpecialEventCard
                        ...
                        icon="🌸"
                        eyebrow="5 · 2 · 0"
                        title="If they became tiny"
                        subtitleActive="2026.5.20 — Spend a tiny afternoon together with them"
                        subtitlePast="2026.5.20 — Relive that tiny afternoon"
                        hintActive="Pick a character to begin"
                        hintPast="Tap a character to relive the record"
                ...
                {visibility.whiteday.show && (
                    <SpecialEventCard
                        ...
                        icon="🍫"
                        title="White Day Special Event"
                        subtitleActive="2026 White Day — DIY a custom chocolate together with them"
                        subtitlePast="2026.3.14 — Relive your custom chocolate"
                        hintActive="Pick a character to begin"
                        hintPast="Tap a character to view the record or make it again"
                ...
                {visibility.valentine.show && (
                    <SpecialEventCard
                        ...
                        icon="💝"
                        title="Valentine's Day Special Push"
                        subtitleActive="2026 Valentine's Day — Hear what they want to say to you"
                        subtitlePast="2026.2.14 — Relive what they said that day"
                        hintActive="Pick a character to begin"
                        hintPast="Tap a character to replay or regenerate"
            ...
            {l520DeleteTargetId && (
                <ConfirmDeleteModal
                    title="Delete 520 Record"
                    charName={characters.find(c => c.id === l520DeleteTargetId)?.name || ''}
                    note="This will delete this character's 520 record, including all related chat messages. This action cannot be undone."
            ...
            {wdDeleteTargetId && (
                <ConfirmDeleteModal
                    title="Delete White Day Record"
                    charName={characters.find(c => c.id === wdDeleteTargetId)?.name || ''}
                    note="This will delete this character's White Day record. This action cannot be undone."
            ...
            {deleteTargetId && (
                <ConfirmDeleteModal
                    title="Delete Valentine's Day Record"
                    charName={characters.find(c => c.id === deleteTargetId)?.name || ''}
                    note="This will delete this character's Valentine's Day record, including stored memories and the corresponding chat messages. This action cannot be undone."
...
export const QixiReplayChoiceModal: React.FC<{
    ...
    <div className="qixi-replay-modal">
        <button type="button" aria-label="Close" className="qixi-replay-modal__scrim" onClick={onCancel} />
        <div className="qixi-replay-modal__panel">
            ...
            <div className="qixi-replay-modal__content">
                ...
                <h3>This dream has<br />already been recorded once.</h3>
                <p>{charName}'s last star-line is still there. Do you want to replay the original content, or recall memories again to generate a new one?</p>
                <div className="qixi-replay-modal__choices">
                    <button type="button" data-qixi-entry-choice="replay" onClick={onReplay}>
                        <span><b>Replay Last Time</b><small>No model call, no duplicate private-chat writes</small></span><i>→</i>
                    </button>
                    <button type="button" data-qixi-entry-choice="fresh" onClick={onFresh}>
                        <span><b>Start a New One</b><small>Recall and generate again; the old record stays until the new one finishes</small></span><i>↗</i>
                    </button>
                </div>
                <button type="button" onClick={onCancel} className="qixi-replay-modal__cancel">Not right now</button>
            </div>
        </div>
    </div>
);
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 animate-fade-in">
        ...
        <div className="relative bg-white rounded-3xl p-6 max-w-xs w-full shadow-2xl">
            <div className="text-center mb-4">
                ...
                <p className="text-xs text-slate-400 mt-2 leading-relaxed">
                    This will delete <span className="font-bold text-slate-600">{charName}</span>'s record. {note}
                </p>
            </div>
            <div className="flex gap-3">
                <button onClick={onCancel} className="flex-1 py-2.5 bg-slate-100 text-slate-500 font-bold rounded-xl active:scale-95 transition-transform text-sm">Cancel</button>
                <button onClick={onConfirm} className="flex-1 py-2.5 bg-red-500 text-white font-bold rounded-xl active:scale-95 transition-transform text-sm">Confirm Delete</button>
            </div>
        </div>
    </div>
);
