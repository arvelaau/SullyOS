    ...
    return (
        <div className="qixi-chat-card" {...interactionProps}>
            ...
            <div className="qixi-chat-card__body">
                ...
                <div className="qixi-chat-card__title">{card.title || 'Starlit Dream Fairytale'}</div>
                ...
                <div className="qixi-chat-card__stats"><span>{String(sceneCount).padStart(2, '0')} Locations</span><span>Magpies Wove the Path</span><span>Promise Fulfilled</span></div>
                {bridgeNames.length > 0 && <div className="qixi-chat-card__nodes">{bridgeNames.map((name, index) => <span key={`${name}-${index}`}>{name}</span>)}</div>}
                <div className="qixi-chat-card__footer"><span>Shared memories written into context</span><time>✦ {new Date(card.timestamp || timestamp || Date.now()).toLocaleDateString()}</time></div>
            </div>
        </div>
    );
};
